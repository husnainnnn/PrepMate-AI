...
const TYPE_MAP: Record<string, { label: string, icon: any, color: string }> = {
  ...
  'report-student': { label: 'Student Report', icon: UserX, color: 'text-rose-500 bg-rose-50' },
  'need-help': { label: 'Need Help', icon: HelpCircle, color: 'text-purple-500 bg-purple-50' },
}

export default function AdminCompanyHelp() {
  ...
  return (
    <AdminLayout>
      <div className="border-b border-[#EAECF0] bg-white px-8 py-4">
        <h1 className="text-lg font-semibold text-[#101828]">Company Help Requests</h1>
        <p className="text-[13px] text-[#667085]">Support tickets and reports from companies</p>
      </div>
      <div className="p-8">
        {loading ? (
          ...
          <div className="rounded-xl border border-[#EAECF0] bg-white py-16 text-center">
            ...
            <p className="text-[12px] text-[#667085]">No pending company requests.</p>
          </div>
        ) : (
          <div className="space-y-3">
            {tickets.map((t: any) => {
              ...
              const detail = t.bugName || t.featureName || t.studentName || t.studentEmail || 'Help Request'
              return (
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex items-start gap-2.5 min-w-0">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="mt-0.5 text-[12px] sm:text-[12.5px] text-[#667085] line-clamp-2">{t.bugDetails || t.featureDescription || t.studentReason || t.helpMessage || 'No details'}</p>
