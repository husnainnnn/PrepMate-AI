...
const NOTIF_ICONS: Record<string, { icon: any; color: string; bg: string }> = {
  new_job:                { icon: Briefcase,    color: 'text-blue-600',  bg: 'bg-blue-50' },
  message:                { icon: MessageSquare, color: 'text-purple-600', bg: 'bg-purple-50' },
  application_shortlisted: { icon: UserCheck,   color: 'text-emerald-600', bg: 'bg-emerald-50' },
  application_rejected:   { icon: XCircle,      color: 'text-red-600',   bg: 'bg-red-50' },
  application_hired:      { icon: Sparkles,     color: 'text-amber-600', bg: 'bg-amber-50' },
  application_review:     { icon: CheckCircle2, color: 'text-cyan-600',  bg: 'bg-cyan-50' },
  ...
  interview_reminder:     { icon: Bell,         color: 'text-orange-600', bg: 'bg-orange-50' },
}
...
export default function StudentNotifications() {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        // 1. Mark all as read first
        await fetch('/api/notifications/read-all', {
          method: 'PATCH',
          headers: { Authorization: `Bearer ${token}` },
        })
      } catch { /* ignore */ }
      // 2. Then fetch — gets clean read state
      await fetchNotifications()
    }

    init()

    // Safety: mark all read when navigating away too
    return () => {
          ...
          <Card className="rounded-xl border-[#EAECF0] shadow-sm">
            <CardContent className="p-12 text-center">
              ...
              <p className="mt-2 max-w-md mx-auto text-[13.5px] text-[#667085]">
                You'll get notified about new job matches, messages, and application updates here.
              </p>
            </CardContent>
          </Card>
        ) : (
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          ...
                          <p className="mt-0.5 text-[13px] text-[#667085] leading-relaxed">
                            {n.message}
                          </p>
                        </div>
