...
import { Check, Crown, Loader2, Briefcase, Users, Zap, TrendingUp } from 'lucide-react'
...
const BENEFITS = [
  ...
  'Review unlimited applicants',
  'Advanced AI screening for candidates',
  'Priority support & faster verification',
  'Detailed analytics & hiring insights',
]

export default function CompanyProPlan() {
  const { token } = useAuth()
  ...
  const handleUpgrade = async (plan: 'monthly' | 'yearly') => {
    ...
    try {
      const res = await fetch('/api/payments/create-checkout-session', {
        ...
        body: JSON.stringify({ plan, role: 'company' }),
      })
  ...
  return (
    <CompanyDashboardLayout>
      ...
      <div className="border-b border-[#EAECF0] dark:border-[#334155] bg-gradient-to-r from-[#F7F9FC] to-white dark:from-[#1E293B] dark:to-[#1E293B] px-8 py-6">
        <div>
          <h1 className="text-lg font-semibold text-[#101828] dark:text-[#F1F5F9]">Upgrade to Enterprise</h1>
          <p className="text-[13px] text-[#667085] dark:text-[#94A3B8]">Scale your hiring with premium recruitment tools</p>
        </div>
      </div>
      ...
      <div className="space-y-6 px-8 py-8">
        ...
        <div className="mx-auto max-w-2xl pt-4">
          <h3 className="text-center text-[15px] font-semibold text-[#101828] dark:text-[#F1F5F9]">Everything you get with Enterprise</h3>
          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            {[
              { icon: Briefcase, title: 'Unlimited Job Postings', desc: 'Post as many jobs as you need to find the right talent' },
              { icon: Users, title: 'Unlimited Applicant Reviews', desc: 'Review every candidate with no caps or limits' },
              { icon: Zap, title: 'AI-Powered Screening', desc: 'Automatically screen and rank applicants with AI' },
              { icon: TrendingUp, title: 'Hiring Analytics', desc: 'Get detailed insights into your recruitment funnel' },
              { icon: Crown, title: 'Priority Support', desc: 'Fast-track verification and dedicated support' },
            ].map((feat, i) => {
